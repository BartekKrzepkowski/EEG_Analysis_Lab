...
subplot(1,EEG.nbchan, kanal); hold on; plot(EEG.times, squeeze(EEG.data(kanal,:,:)));   %plot(dane_na_ośX, dane_na_ośY)
%title('osoba z grupy 6, kanał 3 = elekotrda Cz')   
title(['osoba z grupy ',EEG.subject, ' kanal ', int2str(kanal),'= elekotrda ',EEG.chanlocs(kanal).labels ]);
xlabel('czas [ms]');
ylabel('potencjal [mV]');
...
figure; hold on ;
plot(EEG.times, squeeze(EEG.data(kanal,:,:)));   %plot(dane_na_ośX, dane_na_ośY)
plot(EEG.times, AEP_gr1_av(kanal,:),'k*-');  %plot(dane_na_ośX, dane_na_ośY, 'cechy linii')
xlabel('czas [ms]');
ylabel('potencjal [mV]');
grid on;
